...
@section('content')
 <div class="w-100 text-right mb-3">
     <h5 class="mb-0 float-left" >Payment Invoice Management</h5>
            ...
            <div class="card-body">
                ...
                <div class="row"> 
                    <div class="col-md-8 mx-auto">
                        <form action="{{route('purchase_invoice.submit')}}" method="post">
                            <div class="row"> @csrf
                                <div class="col-md-12">
                                ...
                                <div class="col-md-4">
                                    <p class="mb-0 text-center"><b>PURCHASE INVOICE</b></p>
                                </div>
                                ...
                                <div class="col-md-5">
                                   <table class="w-100 mt-2"  style="">
                                       ...
                                       <tr> <td><label for="">Vendor Name</label></td> <td> 
                                                 <select name="vendor_id" id="vendor_ids" class="inv_inp" required>
                                                     <option value=""></option>
                                   ...
                                   <table class="w-100"  style="" id="vendor_data">
                                       <tr> <td><label for="">Street Address</label></td> <td><input name="address" type="text" class="inv_inp"></td>  </tr>
                                       <tr> <td><label for="">City,State,ZIP</label></td> <td><input name="location" type="text" class="inv_inp"></td>  </tr>
                                       <tr> <td><label for="">Phone No</label></td> <td><input name="phone" type="text" class="inv_inp"></td>  </tr>
                                       <tr> <td><label for="">E-Mail</label></td> <td><input name="email" type="text" class="inv_inp"></td>  </tr>
                                       <tr> <td><label for="">Business Place</label></td> <td><input name="biz_place" type="text" class="inv_inp"></td>  </tr>
                                   </table>
                                </div>
        ...
        <div class="card">
            ...
            <div class="card-body table-responsive">
                ...
                @foreach($trans_docs as $trans_doc) 
                   ...
                   <td>  <a class="btn btn-primary btn-xs btn-block" href="{{route('purchase_invoice.profile', ['doc_id'=>$trans_doc->doc_id])}}"> <span class="fa fa-user"></span> Invoice Profile</a> </td> 
                </tr>
                @endforeach
                    </tbody>
     ...
     <div class="modal fade" id="purchase_invoice_modal">
          ...
          <div class="modal-content">
            {{-- {!! Form::open(['route' => ['invoice.submit_purchase'], 'files' => false, 'id'=>'submit_purchase']) !!} --}}
             <div class="modal-header bg-secondary">  <input type="hidden" name="_method" value="PUT">
              <h4 class="modal-title text-white"> <span class="fa fa-edit"></span> {{__('PURCHASE INVOICE')}} </h4>
      ...
      <!-- /.modal -->
  <script>
    $('#vendor_ids').change(function() { //   console.log($(this).val());
    var vendor_id = $(this).val();  

    var data2send={'vendor_id':vendor_id};       console.log(data2send);
    $('#vendor_data').html('<div class="text-center p-5 mt-4 bg-white"> <img src=" {{ asset('images/preloader1.gif') }} " class="img img-fluid mx-auto" alt=""> </div>');
    
    $.ajaxSetup({ headers: { 'X-CSRF-TOKEN': $("meta[name=csrf-token]").attr('content') } });
    $.ajax({
        url:"{{ route('purchase_invoice.fetch_vendor_data_inv') }}",
        dataType:"text",
        method:"GET",
        data:data2send,
        success:function(resp) {
            $('#vendor_data').html(resp);
        }
    }); 
}); 
  </script>
