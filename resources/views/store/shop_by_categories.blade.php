@extends('layouts.store')
	@section('page_title')
	   {{ config('app.name', "ALUPI ITNL") }} - Shop By Product Category
	@endsection
@section('headers')
                                ...
                                @if (($category->cat_name==$main_category->cat_name)&&($main_category->children))
                                  ...
                                  <div class="input-checkbox">
									...
									<label for="{{$main_category->slug()}}">
                                        ...
                                        @if (($category->cat_name==$main_category->cat_name)&&($main_category->children))
                                            <ul class="sub-category ml-4 mt-2">
                                                @foreach ($main_category->children as $sub_category)
                                                    <div class="input-checkbox">
                                                        {{-- <input type="checkbox" id="{{$sub_category->slug()}}"> --}}
                                                        <label for="{{$sub_category->slug()}}">
                                                            <span></span>
                                                            <a class="sub-category" href="{{route('shop.shop_by_categories', ['cat_id'=>$sub_category->id, 'slug'=>$sub_category->slug(), 'cate'=>'main'])}}">{{$sub_category->cat_name}}</a>
                                                            {{-- <small>(120)</small> --}}
                                                        </label>
                                                    </div>
                                                @endforeach 
